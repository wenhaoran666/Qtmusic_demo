...
Likewidelegate::Likewidelegate(QObject *parent)
...
QSize Likewidelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    Q_UNUSED(option);
    Q_UNUSED(index);
...
void Likewidelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    // 1. 先获取当前项的喜欢状态（假设模型中定义了 LikeRole 角色）
    // bool isLiked = index.data(BaseMusicModel::IsLikedRole).toBool();
    // if (!isLiked) {
    //     // 若未喜欢，则不绘制任何内容（相当于“移除”该项）
    //     return;
    // }
...
bool Likewidelegate::editorEvent(QEvent *event, QAbstractItemModel *model, const QStyleOptionViewItem &option, const QModelIndex &index)
{
    ...
    if (event->type() == QEvent::MouseButtonRelease) {
        ...
        if (likeBtnRect.contains(mouseEvent->pos())) {
            ...
            if (BaseMusicModel *musicModel = static_cast<BaseMusicModel*>(model))
            {
                ...
                musicModel->setLikedState(index.row(), false);
            }
            return true; // 处理后返回
        }
    }
...
// 封装：绘制播放按钮（使用图片）
void Likewidelegate::drawPlayButton(QPainter *painter, const QStyleOptionViewItem &option, const QRect &contentRect) const
{
    // 判断当前项是否被鼠标悬浮（核心逻辑）
...
QRect Likewidelegate::getPlayButtonRect(const QRect &contentRect) const
{
...
// 新增：绘制爱心按钮
void Likewidelegate::drawLikeButton(QPainter *painter, const QStyleOptionViewItem &option,
                                      const QRect &contentRect, const QModelIndex &index) const
{
    ...
    const QPixmap *currentPixmap = nullptr;
        // 项选中或悬浮状态：区分喜欢/非喜欢
        currentPixmap = useHoverStyle ? &m_likeSelectedHover : &m_likeSelected;
...
// 新增：计算爱心按钮区域（固定在右侧）
QRect Likewidelegate::getLikeButtonRect(const QRect &contentRect) const
{
    // 固定在项右侧：右边界 - 偏移量 - 按钮大小
...
// 新增：绘制背景（实现圆角、边距、状态色）
void Likewidelegate::drawBackground(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    Q_UNUSED(index);
...
void Likewidelegate::drawCover(QPainter *painter, const QRect &rect, const QModelIndex &index) const
{
...
void Likewidelegate::drawTextInfo(QPainter *painter, const QRect &rect, const QModelIndex &index) const
{
    // 文本起始X：封面右侧 + 15px间距
...
void Likewidelegate::drawDuration(QPainter *painter, const QRect &rect, const QModelIndex &index) const
{
